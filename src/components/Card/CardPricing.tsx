...
const CardPricing = (props: { el: CardProps }) => {


	return <div className={styles.CardPricing + " " + " card"}
		...
		{props.el.textHtml && props.el.textHtml ? <article className={"description row-start-2"}>
			...
			: <></>}
		{props.el.options && <div className={"options grid gap-2 pt-6"}>{props.el.options.map(el => <div key={el.label} className={"option flex justify-between text-sm border-b border-white py-3"}>
			<span className={"label font-bold"}>{el.label !== "Single" ? el.label + " pack" : el.label}</span>
			<div className={"values"}>{el.old_value && <span className={"line-through text-red-300 pr-1.5"}>${el.old_value}</span>}${el.value}</div>
		</div>) }</div>}
